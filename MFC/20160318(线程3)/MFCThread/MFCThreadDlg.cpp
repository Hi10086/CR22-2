...
{
  CMFCThreadDlg *lpDlg = (CMFCThreadDlg*)lpParam;
  while(true)
  {
    OutputDebugString("run...");
    lpDlg->UpdateData(TRUE);
...
void CMFCThreadDlg::OnButton1() 
{
  CWinThread* lpThread = AfxBeginThread(ThreadProc, this);
  lpThread->SuspendThread();
  lpThread->ResumeThread();
}

void CMFCThreadDlg::OnButton2() 
{
  CMyThread* lpThread = AfxBeginThread(RUNTIME_CLASS(CMyThread));
  //WaitForSingleObject(lpThread->m_hThread
}
