...
void CCallDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON3, m_MyButton);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CCallDlg, CDialog)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_STATIC, OnBtnStatic)
	ON_BN_CLICKED(IDC_BTN_SHARED, OnBtnShared)
...
void CCallDlg::OnBtnStatic() 
{
  FARPROC pfnShowMsg = NULL;

  HMODULE hDll = LoadLibrary("../StaticDll/Debug/StaticDll.dll");
  //check
  
  pfnShowMsg = GetProcAddress(hDll, "ShowDlg");
  //check
...
void CCallDlg::OnBtnShared() 
{
  FARPROC pfnShowMsg = NULL;
  
  HMODULE hDll = LoadLibrary("../SharedDll/Debug/SharedDll.dll");
  //check
